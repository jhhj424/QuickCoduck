			...
			<div align="center">
				...
				<div align="center" onmouseover="this.style.backgroundColor=''"
					...
					<div align="right" class="listcount_div">
						<b class="listcount_b"> <i class="fa fa-asterisk w3-text-teal"></i>
							글개수: ${listcount}
						</b>
					</div>
		...
		<c:if test="${ON != 1}">
			<c:forEach var="board" items="${boardlist}">
				<div align="center"
					...
					onmouseout="this.style.backgroundColor=''" class="content">
					<div align="left">
						<c:if test="${! empty board.fileurl}">
							<a href="../file/${board.fileurl}"> <span class="span"><i
									class="fa fa-file-image-o w3-margin-right"></i>Image</span>
							</a>
						</c:if>
						<c:if test="${empty board.fileurl}">
							<span class="span2"><i
								class="fa fa-frown-o w3-margin-right"></i>Image</span>
						</c:if>
						<c:if test="${param.type == 1 }">
							<c:if test="${board.duckcnt > 10}">
								<span class="tag_duck"><i class="fa fa-github"></i> Hot!</span>
							</c:if>
						</c:if>
						<!-- 이벤트! -->
						<c:if test="${param.type == 3 }">
							<c:if test="${board.maxperson-1 <=  board.nowperson}">
								<span class="tag4"><i class="fa fa-certificate"></i>
									마감임박!</span>
							</c:if>
							<c:if test="${board.maxperson == board.nowperson}">
								<span class="tag5"><i class="fa fa-rocket"></i> 모집종료</span>
							</c:if>
							<c:if test="${board.nowperson == 0}">
								<span class="tag_new"><sup>New</sup> 신규등록!</span>
							</c:if>
						</c:if>
						<!-- 임의로 이벤트 하나 만든거임 -->
						<c:if test="${board.boardtype==1}">
							<c:if test="${board.recmd >= 11 }">
								<span class="recmd_best"><i class="fa fa-heart"></i> 추천 :
									${board.recmd}</span>
							</c:if>
							<c:if test="${board.recmd <= 10 || board.recmd == 0 }">
								<span class="recmd"><i class="fa fa-thumbs-o-up"></i> 추천
									: ${board.recmd}</span>
							</c:if>
						</c:if>
						<h3 class="h3">
					...
					<div align="left" class="thumb">
						<b
							style="margin: 5px 5px 5px 5px; font-weight: bold; color: #000 !important;"><i
							class="fa fa-low-vision"></i>내용 미리보기</b><br>
					...
					<div align="right" class="right">
						...
						&nbsp;&nbsp;<br> <b>조회수:</b>&nbsp;&nbsp;<sup class="readcnt">${board.readcnt}</sup>
						&nbsp;&nbsp;
					</div>
					<div align="right" class="right">
						<b>선택기술:</b>&nbsp;&nbsp;
					...
					<div align="right" class="right">
						...
						<c:if test="${!empty board.usetech }">
							<c:forEach var="usetech" items="${board.usetech}" varStatus="g"
								begin="0">
					...
					<div>
						<c:if test="${pageNum > 1}">
							<a href="javascript:list(${pageNum -1})"><i
								class="fa fa-arrow-left w3-large"></i>이전</a>
						</c:if>
						<c:if test="${pageNum <= 1}">
							<i class="fa fa-arrow-left w3-large"></i>이전</c:if>
						...
						<c:if test="${pageNum < maxpage}">
							<a href="javascript:list(${pageNum +1})">다음<i
								class="fa fa-arrow-right w3-large"></i></a>
						</c:if>
						<c:if test="${pageNum >= maxpage}">다음<i
								class="fa fa-arrow-right w3-large"></i>
						</c:if>
					</div>
				</div>
			</div>
		...
		<div>
			<div align="center">
				...
				<div align="center" onmouseover="this.style.backgroundColor=''"
					onmouseout="this.style.backgroundColor=''">
					<div align="right" class="listcount_div">
						<b class="listcount_b"> <i
							class="fa fa-cloud-upload w3-text-teal"></i> 나만의소스개수: ${mycount}
						</b>
					</div>
					<div align="center" style="margin-top: 5px;">
						<form action="find.duck?type=${param.type}" method="post"
			...
			<c:forEach var="board" items="${mylist}">
				<div align="center"
					...
					<div align="left" class="thumb">
						<b
							style="margin: 5px 5px 5px 5px; font-weight: bold; color: #000 !important;"><i
							class="fa fa-low-vision"></i>내용 미리보기</b><br>${board.content }</div>
					<div align="right" class="right">
						<b>등록일자:</b>
						<fmt:formatDate value="${board.regdate}"
							pattern="yyyy/MM/dd hh:mm" />
